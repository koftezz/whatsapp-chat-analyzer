...
def _parse_whatsapp_file(file_bytes: bytes) -> pd.DataFrame:
    """
    Internal function to parse WhatsApp file bytes with error handling.
    
    This function is not cached and is used for testing. It wraps ChatMiner's
    WhatsApp parser and converts various errors into ParseError with helpful messages.
    
    Args:
        file_bytes: Raw bytes of the WhatsApp export file
        
    Returns:
        DataFrame with parsed messages (basic columns: timestamp, author, message)
    ...
    try:
        df = _parse_with_chatminer(file_bytes, WhatsAppParser)
...
def read_file(file) -> pd.DataFrame:
    """
    Read and parse a WhatsApp, Signal, or sigtop text export file.

    Tries to detect the format and parse with the appropriate parser:
    - Sigtop exports are detected by their "Conversation:" header and parsed directly
    - Other files are tried with ChatMiner's WhatsApp parser (with detailed error handling)
    - If WhatsApp parsing fails, Signal parser is tried as a fallback
    
    Basic features (weekday, hour, words, letters) are added to all successfully parsed data.
    ...
    # Try to decode as text for sigtop detection
    try:
    ...
    with st.spinner('This may take a while. Wait for it...'):
        ...
        # Try sigtop format first if it looks like one
        ...
        # Final check: if we still have no data, raise an error
        if df.empty:
            raise ParseError(
                "Could not parse this chat export file. "
                "Please ensure it's a valid WhatsApp, Signal, or sigtop export.\n\n"
                "Supported formats:\n"
                "- WhatsApp: Export from chat menu → 'Export Chat' → without media\n"
                "- Signal: Export using chat-miner or compatible tools\n"
                "- Sigtop: Export using `sigtop msg` command"
            )
